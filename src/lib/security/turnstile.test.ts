...
describe("Turnstile server verification", () => {
  ...
  it("fails closed in production when expected hostname is missing", async () => {
  ...
  it("rejects a missing token before contacting Cloudflare", async () => {
    ...
    const result = await verifyTurnstile({
      ...
      environment: "production",
      secret: "secret",
      expectedHostname: "gereja.example",
      fetcher,
    });
  ...
  it("allows Cloudflare official test-key metadata only outside production", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
        new Response(
  ...
  it("does not let test metadata bypass production action validation", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
        new Response(
