...
export async function POST(request: Request) {
  ...
  const auth = await requireAdminPermission("livestreams.manage");
  ...
  if (!(file instanceof File) || file.size === 0) {
    return NextResponse.json(
      { message: "Pilih thumbnail untuk diunggah" },
      { status: 400 },
    );
  }
  ...
  if (!ALLOWED_IMAGE_TYPES.has(file.type)) {
    return NextResponse.json(
      { message: "Thumbnail harus berformat JPG, PNG, atau WebP" },
      { status: 422 },
    );
  }
  ...
  if (uploadError) {
    return NextResponse.json(
      { message: "Thumbnail tidak dapat diunggah ke penyimpanan" },
      { status: 500 },
    );
  }
  ...
  if (error) {
    ...
    return NextResponse.json(
      { message: "Metadata thumbnail tidak dapat disimpan" },
      { status: 500 },
    );
  }
