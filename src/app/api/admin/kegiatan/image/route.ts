...
export async function POST(request: Request) {
  ...
  const auth = await requireAdminPermission("events.manage");
  ...
  if (!(file instanceof File) || file.size === 0) {
    return NextResponse.json(
      { message: "Pilih poster untuk diunggah" },
      { status: 400 },
    );
  }
  ...
  if (!ALLOWED_IMAGE_TYPES.has(file.type)) {
    return NextResponse.json(
      { message: "Poster harus berformat JPG, PNG, atau WebP" },
      { status: 422 },
    );
  }
  ...
  if (file.size > MAX_IMAGE_BYTES) {
    return NextResponse.json(
      { message: "Ukuran poster maksimal 5 MB" },
      { status: 422 },
    );
  }
  ...
  if (uploadError) {
    return NextResponse.json(
      { message: "Poster tidak dapat diunggah ke penyimpanan" },
      { status: 500 },
    );
  }
  ...
  if (error) {
    ...
    return NextResponse.json(
      { message: "Metadata poster tidak dapat disimpan" },
      { status: 500 },
    );
  }
